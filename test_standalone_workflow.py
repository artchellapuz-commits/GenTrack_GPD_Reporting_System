#!/usr/bin/env python3
"""
Test the standalone email workflow function
"""
import requests
import json
import time
...
# Test data
test_data = {
    "signatory_name": "STANDALONE TEST",
    "role": "Test Role",
    "email": "standalonetest@example.com",
    "justification": "Testing the standalone email workflow function"
}
...
print("Testing standalone email workflow...")
        ...
        if access_token:
            ...
            if response.status_code == 201:
                ...
                if auth_response.status_code == 200:
                    ...
                    standalone_auth = None
                    for auth in auths:
                        if auth['signatory_name'] == 'STANDALONE TEST':
                            standalone_auth = auth
                            break
                    
                    if standalone_auth:
                        print(f"\n✅ Authorization auto-created:")
                        print(f"ID: {standalone_auth['id']}")
                        print(f"Active: {standalone_auth['is_active']}")
                        print(f"Valid: {standalone_auth['is_valid']}")
                        ...
                        if req_response.status_code == 200:
                            ...
                            standalone_request = None
                            for req in requests_data:
                                if req['signatory_name'] == 'STANDALONE TEST':
                                    standalone_request = req
                                    break
                            
                            if standalone_request:
                                print(f"\n✅ Request status:")
                                print(f"Status: {standalone_request['status']} ({standalone_request['status_display']})")
                                print(f"Admin Notes: {standalone_request['admin_notes']}")
                                
                                if standalone_request['status'] == 'APPROVED':
                                    print("\n🎉 COMPLETE SUCCESS!")
                                    ...
                                    print("\n🎯 THE API WORKFLOW IS FINALLY WORKING!")
                                else:
                                    print(f"\n⚠️ Request status: {standalone_request['status']}")
                    else:
